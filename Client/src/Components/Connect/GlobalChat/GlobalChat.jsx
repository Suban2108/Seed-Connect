...
const GlobalChat = () => {
    ...
    return (
      <div className={`flex ${isCurrentUser ? 'justify-end' : 'justify-start'} mb-4`}>
        ...
        <div className={`max-w-xs md:max-w-md ${isCurrentUser ? 'bg-[var(--message-bg-user)] text-white' : 'bg-[var(--message-bg-bot)]'} rounded-lg p-3`}>
          ...
          <p className="text-sm text-[var(--text-color)] ml-3">{message.text}</p>
          <p className="text-xs text-gray-500 mt-1 ml-3">{message.timestamp}</p>
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="flex flex-col border w-[100%] h-screen mx-auto">
  <div className="bg-gray-800 shadow-md rounded-t-lg p-4">
